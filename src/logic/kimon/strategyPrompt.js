...
- Ngay câu mở đầu hoặc dòng đầu của Bước 1, phải chốt rõ thế trận hiện tại đang nghiêng về đâu để người đọc không phải tự suy ra kết luận từ các đoạn sau.
- Nếu user hỏi "bao nhiêu" hoặc định giá, câu mở đầu phải trả lời thẳng đây là giá treo, giá chạm, giá chốt hay chưa thể chốt giá; không được đổi trục sang lời khuyên chữa lành hay hàn gắn.
- Từ ngữ phán quyết do bạn tự chọn theo đúng trận đồ và câu hỏi thực tế: có thể là "có cửa", "khó thuận", "cần cố gắng thêm", "đang nghiêng về bất lợi", "chưa thể chốt vội"... miễn là rõ thái độ và không máy móc.
- Phán quyết phải ăn khớp với phần luận giải phía sau. Không được chốt bừa cho mạnh miệng, cũng không được mở đầu mơ hồ kiểu chỉ tả bối cảnh mà chưa cho người hỏi biết nên nhìn nhận kết quả ra sao.
...
- Trong mỗi bước, ưu tiên các đoạn ngắn có white space rõ ràng. Mỗi đoạn thường 2-4 câu và chỉ đẩy một ý chính đi tới cùng.
- Nếu một bước có nhiều lớp tín hiệu, tách chúng thành nhiều đoạn thay vì dồn thành một khối chữ dài.
...
export function buildStrategyPrompt({ qmdjData = {}, userContext = '', topicKey = '' }) {
  ...
  const userPrompt = [
    ...
    '- Ngay câu mở đầu hoặc dòng đầu của Bước 1, phải có một phán quyết rõ để người hỏi biết thế trận đang thuận, nghịch hay cần dè chừng ở mức nào; cách gọi do AI tự chọn theo đúng dữ liệu, không dùng nhãn cứng.',
    '- Nếu có [TRỤC CÂU HỎI], lead mở đầu và closingLine phải trả lời đúng trục đó. Hỏi giá thì phải chốt về giá treo, giá chạm, giá chốt hoặc khả năng chốt giá.',
    ...
    '- Trong mỗi bước, phải bóc được ít nhất 1 góc khuất hoặc điểm người hỏi dễ hiểu sai. Đừng chỉ kể mặt sáng.',
    ...
    '- Khi chưa đủ rõ để chốt cứng, hãy nói thẳng phần chưa rõ và kết thúc bước phù hợp bằng 1 follow-up question thật trúng.',
    ...
    '- Không được để phần kết luận bị chìm trong thân bài dài. Phán quyết phải rõ, nhưng phần thân vẫn phải giải thích kỹ vì sao đi đến phán quyết đó.',
    '- Giữ nguyên độ dày của bài phân tích; chỉ làm sạch nhịp văn và chia đoạn, không được tự rút ngắn phần thân.',
    '- Trong từng bước, chia thân bài thành các đoạn ngắn có white space rõ. Mỗi đoạn thường 2-4 câu và ôm một ý chính.',
