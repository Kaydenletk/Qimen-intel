...
export function buildCareerInsight(context, topicKey) {
  ...
  if (context.flags.VOID || context.flags.FU_YIN || context.flags.FAN_YIN) rawScore -= 4;
  ...
  const headline = headlineBySignal(scoreSignal, isNegotiation);
  const coreMessage = isNegotiation
