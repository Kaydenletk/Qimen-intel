...
assert.match(strategyPrompt.systemPrompt, /\[SCANNABILITY - ĐỊNH DẠNG QUÉT NHANH\]/);
assert.match(strategyPrompt.systemPrompt, /\[4-TẦNG DẪN CHUYỆN - MẶC ĐỊNH CHO THINKING\]/);
assert.match(strategyPrompt.systemPrompt, /Vị thế của bạn: đọc Nhật Can và cung Marker Ngày/i);
assert.match(strategyPrompt.systemPrompt, /Đối phương: đọc Trực Phù\/Trực Sử/i);
assert.match(strategyPrompt.systemPrompt, /Sự việc & Sự thật: đọc Cung Giờ/i);
assert.match(strategyPrompt.systemPrompt, /markdown heading kiểu "###"|markdown heading kiểu/i);
assert.match(strategyPrompt.systemPrompt, /Tránh đoạn văn dài quá 3 dòng/i);
assert.match(strategyPrompt.systemPrompt, /\*\*Dừng lại ngay lập tức:\*\*/i);
assert.match(strategyPrompt.systemPrompt, /Tuyệt đối không gửi lời chào, không dựng khung ký tự kiểu \[---\], không lặp lại tiêu đề "Kymon nè"/i);
assert.match(strategyPrompt.systemPrompt, /dòng đầu là lời phán in đậm; các mục lớn dùng "###"/i);
assert.match(strategyPrompt.systemPrompt, /CẤM dùng tên kỹ thuật trần trụi như "### Cảnh Môn \+ Thiên Anh", "### Thái Âm", "### Kinh Môn"/i);
...
assert.match(strategyPrompt.systemPrompt, /\[NGÔN NGỮ CHIẾN LƯỢC & THỰC TẾ\]/);
assert.match(strategyPrompt.systemPrompt, /Áp lực ngàn cân/i);
assert.match(strategyPrompt.systemPrompt, /Nội lực rỗng tuếch/i);
assert.match(strategyPrompt.systemPrompt, /Cửa sáng/i);
assert.match(strategyPrompt.systemPrompt, /Hố tử thần/i);
assert.match(strategyPrompt.systemPrompt, /Miếng phô mai miễn phí chỉ có trong bẫy chuột/i);
...
assert.match(strategyPrompt.userPrompt, /lời phán triết lý kiểu \[hình ảnh ẩn dụ\] \+ \[hệ quả tàn khốc\]/i);
assert.match(strategyPrompt.userPrompt, /chia phần bằng "###" hoặc các subtitle in đậm/i);
assert.match(strategyPrompt.userPrompt, /Vị thế của bạn -> Đối phương -> Sự việc & Sự thật/i);
assert.match(strategyPrompt.userPrompt, /Trong nhịp "Vị thế của bạn", phải soi Nhật Can và cung Marker Ngày/i);
assert.match(strategyPrompt.userPrompt, /Trong nhịp "Đối phương", phải soi Trực Phù hoặc Trực Sử/i);
assert.match(strategyPrompt.userPrompt, /Trong nhịp "Sự việc & Sự thật", phải kéo Cung Giờ vào/i);
assert.match(strategyPrompt.userPrompt, /sub-title kiểu "### Sự thật tàn nhẫn"|sub-title kiểu/i);
assert.match(strategyPrompt.userPrompt, /Tuyệt đối không gửi lời chào, không dùng khung ký tự như \[---\], và không lặp lại tiêu đề "Kymon nè"/i);
assert.match(strategyPrompt.userPrompt, /không dùng tên kỹ thuật trần như "### Cảnh Môn \+ Thiên Anh" hay "### Thái Âm"/i);
...
assert.match(strategyPrompt.userPrompt, /thay "Môn Bức" bằng "Áp lực ngàn cân"/i);
...
assert.match(strategyPrompt.userPrompt, /\*\*Dừng lại ngay lập tức:\*\*/i);
assert.match(strategyPrompt.userPrompt, /\*\*Khóa ví tiền:\*\*/i);
assert.match(strategyPrompt.userPrompt, /\*\*Cắt đứt liên lạc:\*\*/i);
...
assert.match(strategyPrompt.userPrompt, /Tránh đoạn quá 3 dòng/i);
